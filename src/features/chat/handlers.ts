...
export const handleSendChatFn = () => async (text: string) => {
  ...
  if (ss.externalLinkageMode) {
    ...
    if (
      ss.responseSettings?.enableCharacterLimit &&
      ss.responseSettings?.characterLimit > 0
    ) {
      const characterLimit = ss.responseSettings.characterLimit;
      updatedSystemPrompt =
        `あなたの回答は必ず${characterLimit}文字以内にしてください。これは厳密な制限です。\n\n` +
...
5. 説明が長くなりそうな場合は、最も重要な情報のみを含めてください`;
      ...
      updatedSystemPrompt = systemPrompt.replace(
        '{{RESPONSE_LENGTH}}',
        '制限なし'
      );
    }
