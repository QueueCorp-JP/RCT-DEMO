  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (!systemPrompt.includes('参考資料（製品説明用）：')) {
        ...
        newPrompt = systemPrompt.replace(regex, `参考資料（製品説明用）：\n${data.text}`);
      }
      ...
      settingsStore.setState({
        ...
        pdfSettings: {
          ...
          pdfHistory: [pdfEntry, ...settingsStore.getState().pdfSettings.pdfHistory]
        }
      });
      ...
      >
        <div className="flex flex-col items-center justify-center pt-5 pb-6">
          <div className="relative mb-4">
            ...
            `}></div>
            <svg xmlns="http://www.w3.org/2000/svg" className="relative w-12 h-12 text-blue-600 transition-transform duration-300 transform group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
          </div>
          <p className="mb-2 text-sm text-gray-800 font-medium">
            <span className="text-blue-600 font-bold">クリックしてアップロード</span>
            {' '}または drag & drop
          </p>
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-white/80 backdrop-blur-sm rounded-xl">
          <div className="bg-white shadow-xl rounded-2xl px-8 py-5">
            <div className="flex items-center space-x-4">
              ...
              <span className="text-sm font-semibold text-gray-800">アップロード中...</span>
            </div>
          </div>
        </div>
      )}
